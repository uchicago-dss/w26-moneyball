...
# Features where "smaller is better" (we will flip their sign)
DEFENSE_BAD_FEATURES = [
...
def aggregate_country_stats(country_data: pd.DataFrame) -> pd.DataFrame:
    # Force numeric for everything except identifiers
    df = country_data.copy()
    df["matches_played"] = pd.to_numeric(df["matches_played"], errors="coerce")

    # Make sure these exist and are numeric if present
    for col in ATTACK_FEATURES + DEFENSE_FEATURES + DEFENSE_BAD_FEATURES:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    # Aggregate by country_code
    agg_dict = {"matches_played": "sum"}

    # Sum count-like stats, mean rate-like stats
    # If a column is already "per_match" or "per_game", averaging is reasonable.
    for col in ATTACK_FEATURES + DEFENSE_FEATURES + DEFENSE_BAD_FEATURES:
        if col not in df.columns:
            continue
        if ("per_match" in col) or ("per_game" in col) or (col == "average_possession"):
            agg_dict[col] = "mean"
        else:
            agg_dict[col] = "sum"

    agg = (
        df.dropna(subset=["country_code", "matches_played"])
          .groupby("country_code", as_index=False)
          .agg(agg_dict)
    )

    # Convert summed “totals” into per-match rates where it makes sense
    # (skip columns already per_match/per_game/average_possession)
    for col in ATTACK_FEATURES + DEFENSE_FEATURES + DEFENSE_BAD_FEATURES:
        if col not in agg.columns:
            continue
        if ("per_match" in col) or ("per_game" in col) or (col == "average_possession"):
            continue
        agg[f"{col}_per_match"] = agg[col] / agg["matches_played"]
...
def compute_attack_defense(agg: pd.DataFrame) -> tuple[pd.DataFrame, float]:
    ...
    # Build a unified feature list, using per_match versions when we created them
    attack_cols: list[str] = []
    for col in ATTACK_FEATURES:
        if col in out.columns:
            attack_cols.append(col)
        elif f"{col}_per_match" in out.columns:
            attack_cols.append(f"{col}_per_match")
    ...
    for col in DEFENSE_FEATURES:
        if col in out.columns:
            ...
            defense_cols.append(f"{col}_per_match")

    defense_bad_cols: list[str] = []
    for col in DEFENSE_BAD_FEATURES:
        if col in out.columns:
            defense_bad_cols.append(col)
        elif f"{col}_per_match" in out.columns:
            defense_bad_cols.append(f"{col}_per_match")

    # Z-score everything so they’re comparable
    for col in set(attack_cols + defense_cols + defense_bad_cols):
        out[col] = zscore(out[col])

    # Composite scores (equal weights by default)
    out["attack_score"] = out[attack_cols].mean(axis=1) if attack_cols else 0.0
    out["defense_score"] = out[defense_cols].mean(axis=1) if defense_cols else 0.0
    out["defense_bad_score"] = out[defense_bad_cols].mean(axis=1) if defense_bad_cols else 0.0

    # Bigger attack_score = better attack
    # Bigger defense_bad_score = worse defense -> subtract it
    out["attack"] = np.exp(out["attack_score"] * 0.5)
    out["defense"] = np.exp(-(out["defense_score"] - out["defense_bad_score"]))  # lower is better defense

    # Baseline goals per match
    # Prefer your goals column if present
    if "goals_scored_per_match" in out.columns:
        base_goals = out["goals_scored_per_match"].mean()
    elif "goals_scored_per_match" in agg.columns:
        base_goals = agg["goals_scored_per_match"].mean()
    else:
        base_goals = 1.2  # fallback

    base_goals = float(base_goals) if base_goals > 0 else 1.2
    return out, base_goals
